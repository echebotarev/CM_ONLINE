...
export const templatesSelector = createSelector(
  ...
  templates => templates.valueSeq().toArray()
);

// получаем список кнопок
export const filtratedButtonsSelector = createSelector(
  buttonsSelector,
  currentTemplateSelector,
  (buttons, currentTemplate) => {
    return buttons.filter(button => button.template === currentTemplate);
  }
);

// получаем конкретную кнопку
export const filtratedButtonSelector = createSelector(
  buttonsSelector,
...
export const filtratedTemplatesSelector = createSelector(
  templatesSelector,
  templates => templates
);

// получаем конкретный темплэйт. Используется в EditorContent
export const filtratedTemplateSelector = createSelector(
  templatesSelector,
  currentTemplateSelector,
  (templates, currentTemplate) => {
    let template = templates.filter(
      template => template._id === currentTemplate
    );
    return template[0];
  }
);
