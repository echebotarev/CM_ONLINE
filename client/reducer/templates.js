...
export default (state = defaultState, action) => {
  ...
  switch (type) {
    ...
    case ADD_TEMPLATE + SUCCESS:
      return state
        ...
        .updateIn(["entities"], entities =>
          entities.concat(arrToMap([payload.response], TemplateRecord))
        );
    ...
    case SAVE + TEMPLATE + FAIL:
      return state.updateIn(["entities", error.payload.id], () => editableData);
    ...
    case LOAD_TEMPLATES + SUCCESS:
      return state
        ...
        .setIn(
          ["entities"],
          arrToMap(payload.response.templates, TemplateRecord)
        );
