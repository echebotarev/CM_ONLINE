...
class ColorPickerInput extends Component {
	...
	componentDidMount() {
		let opacity = this.props.opacity || this.props.opacity === 0
	...
	componentWillReceiveProps(nextProps) {
		let opacity = nextProps.opacity || nextProps.opacity === 0
			? nextProps.opacity / 100 : 1,
				...
				>
					<div className="color-picker-wrapper">
						<div
							...
							style={{backgroundColor: this.state.background, opacity: this.state.opacity}}
						></div>
					</div>
				</div>
			</div>
